    ...
    server.setRequestHandler(types_js_1.CallToolRequestSchema, async (request) => {
        ...
        for (const requiredParameter of requiredParameters) {
            if (!(requiredParameter in args)) {
                throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, `Missing required parameter: ${requiredParameter}`);
            }
        }
        try {
            ...
            throw new types_js_1.McpError(types_js_1.ErrorCode.InternalError, `API error: ${error.message}`);
        }
    });
}
async function createWebServer() {
    const app = (0, express_1.default)();
    const tools = await (0, tools_js_1.discoverTools)();
    // Middleware

 ...
 * @param {string} args.bid_strategy - The bidding strategy to use (e.g., COST_CAP).
 * @param {string} [args.stop_time] - The time to stop the campaign (optional).
 * @param {string} [args.daily_budget] - The daily budget for the campaign (optional).
 * @returns {Promise<Object>} - The result of the budget and bid update.
 */
const executeFunction = async ({ campaign_id_traffic_bid, lifetime_budget, bid_strategy, stop_time, daily_budget }) => {
  const baseUrl = ''; // Base URL will be provided by the user
  const token = process.env.FACEBOOK_MARKETING_API_API_KEY;
  try {
    // Construct the URL with query parameters
    const url = new URL(`${baseUrl}/${campaign_id_traffic_bid}`);
    url.searchParams.append('lifetime_budget', lifetime_budget);
    ...
    return { error: 'An error occurred while updating budget and bid.' };
  }
};

/**
 * Tool configuration for updating budget and bid strategy on Facebook.
 * @type {Object}
 */
const apiTool = {
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          bid_strategy: {
            type: 'string',
            description: 'The bidding strategy to use (e.g., COST_CAP).'
          },
          stop_time: {
            type: 'string',
          ...
          daily_budget: {
            type: 'string',
            description: 'The daily budget for the campaign (optional).'
          }
        },

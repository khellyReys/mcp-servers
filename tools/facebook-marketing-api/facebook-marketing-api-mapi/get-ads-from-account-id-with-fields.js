...
const executeFunction = async ({ account_id }) => {
  const baseUrl = 'https://graph.facebook.com/v12.0'; // Facebook Graph API base URL
  const token = process.env.FACEBOOK_MARKETING_API_API_KEY;

  try {
    // Construct the URL for the request
    const url = `${baseUrl}/act_${account_id}/ads?fields=id,name,bid_amount,adset_id,creative{effective_instagram_story_id,effective_instagram_media_id,instagram_permalink_url},status,effective_status,created_time,updated_time,tracking_specs,conversion_specs,ad_review_feedback,adlabels,issues_info,conversion_domain,campaign_id`;
    ...
    console.error('Error retrieving ads from account:', error);
    return { error: 'An error occurred while retrieving ads from the account.' };
  }
};
...
const apiTool = {
  ...
  definition: {
    ...
    function: {
      ...
      description: 'Get ads from a specified Facebook Ad Account.',
      parameters: {
        type: 'object',
        properties: {
          account_id: {
